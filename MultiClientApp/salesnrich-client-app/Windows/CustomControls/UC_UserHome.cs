...
namespace SNR_ClientApp.Windows.CustomControls
{
	public partial class UC_UserHome : UserControl
	{
		...
		public UC_UserHome()
		{
			...
			// Check if nothing is selected and companies list has items

			companySelect.DataSource = LoadCompanyNames.LoadCompanyNamesoftally();
			if (companySelect.SelectedIndex == -1 && LoadCompanyNames.LoadCompanyNamesoftally().Count > 0)
			{
		...
		private void companySelect_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			IsFirstTimeUpload = ApplicationProperties.properties.GetValueOrDefault("isFirstTimeUpload").ToString();
		}
		...
		private void UC_UserHome_Load(object sender, EventArgs e)
		{

		}

		private void chk_productProfile_CheckedChanged(object sender, EventArgs e)
		{
