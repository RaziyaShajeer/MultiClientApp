...
namespace SNR_ClientApp.Services
{
    internal class ProductProfileService
    {
        ...
        private String ProductIgstName;
        private String ProductCessName;
        CompanyStockItemXml CompanyStockItemXml;
        ProductProfileParser ProductProfileParser;
		public ProductProfileService()
		{
            CompanyStockItemXml = new CompanyStockItemXml();
			idClentApp = ApplicationProperties.userinitialproperty.GetValueOrDefault("idclientapp").ToString();
			GSTNames = ApplicationProperties.properties.GetValueOrDefault("tally.gst").ToString();
			ProductCessName = ApplicationProperties.properties.GetValueOrDefault("tally.productCESS").ToString();
	 ProductIgstName = ApplicationProperties.properties.GetValueOrDefault("tally.productIGST").ToString();
            ProductProfileParser = new ProductProfileParser();  
        }
		...
		internal async Task getFromTallyAndUploadAsync(bool isoptimised)
        {
            try
            {
                ...
                tallyRequest = CompanyStockItemXml.getCompanyStockItemXml();
                ...
                List<ProductProfileDTO> allProductProfilespTally = await ProductProfileParser.getProductprofiles(data);
